...
def get_obj_prd_vecs(dataset_name):
    word2vec_model = gensim.models.KeyedVectors.load_word2vec_format(
        cfg.DATA_DIR + '/../vrd/data/word2vec_model/GoogleNews-vectors-negative300.bin', binary=True)
    ...
    if dataset_name.find('vrd') >= 0:
        with open(cfg.DATA_DIR + '/../vrd/data/vrd/objects.json') as f:
            obj_cats = json.load(f)
        with open(cfg.DATA_DIR + '/../vrd/data/vrd/predicates.json') as f:
            prd_cats = json.load(f)
    elif dataset_name.find('vg') >= 0:
        with open(cfg.DATA_DIR + '/../vrd/data/vg/objects.json') as f:
            obj_cats = json.load(f)
        with open(cfg.DATA_DIR + '/../vrd/data/vg/predicates.json') as f:
            prd_cats = json.load(f)
    else:
